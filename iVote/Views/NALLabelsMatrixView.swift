...
protocol NALLabelsMatrixViewDelegate: AnyObject {
...
class NALLabelsMatrixView : UIView {
  ...
  var numRows: Int = 0
  var dy: Int = 0
  weak var delegate: NALLabelsMatrixViewDelegate?
  ...
  func addRecord(record:Array<String>){
    ...
    for i in 0..<record.count {
      ...
      let column:UILabel = UILabel(frame: cellRect)
      ...
      if(self.numRows == 0){
        ...
        let tapGesture = UITapGestureRecognizer.init(target: self, action: #selector(self.recordTapAction(_:)))
        column.addGestureRecognizer(tapGesture)
        column.tag = numRows
      }
      let attrText:NSAttributedString = NSAttributedString.init(string: record[i], attributes: [NSAttributedString.Key.paragraphStyle: paragraphStyle])
  ...
  @objc func recordTapAction(_ tapGestureRecognizer: UITapGestureRecognizer) {
    if let label = tapGestureRecognizer.view as? UILabel {
      self.delegate?.labelsMatrixView(self, didSelectRowAt: label.tag)
    }
  }
}
